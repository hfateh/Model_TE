...
# this code is similar to Chad's because it uses the same Boundary
# conditions and same ways to
# left this code here on Feb 28, 2013
# This is when we talked about fill fraction in detail
# I have to now change my q-h and q-c
# Everything works fine here, but fill fraction is not being used
# correctly
...
class TE_Pair(object):
	...
	fill_fraction = self.fill_fraction
        ...
        # # New set of formulas for areas
        # self.area = (5.e-3) ** 2
        # self.Ptype.area = (
        #     (self.fill_fraction * self.area)/(1. +
        #     self.leg_area_ratio)
        #     )
        # self.Ntype.area = (
        #     self.Ptype.area * self.leg_area_ratio
        #     )
        # self.area_void = (
    ...
    def solve_te_pair_once(self):
        ...
        """ """
        print "Ntype.q_h guess is ", self.Ntype.q_h
        print "Ptype.q_h guess is ", self.Ptype.q_h
    ...
    def get_error(self, knob_arr):
        ...
        self.T_c_error = self.Ntype.T_c - self.Ptype.T_c
        ...
        self.error = (
            np.array([self.T_c_error, self.q_h_error, self.q_c_error, self.I_error]).flatten()
            )
        return self.error
    ...
    def solve_te_pair(self):
        ...
        # thermal efficiency (%)
        self.eta_thermal = 100 * (self.P / self.q_h)
...
#are trying to optimize.
    ...
    def get_minpar1(self, apar):
        ...
        if (apar <= 0.).any():
            ...
            #minpar = np.abs(self.eta_thermal) ** 3. + 10
            print "Encountered impossible value."

        else:
            minpar = - self.P
            #minpar = - self.eta_thermal 
    ...
    def optimize1(self):
        ...
        self.x0 = (
            np.array([self.R_load_total, self.leg_area_ratio,
...
#             self.Ntype.area) + ((self.T_h_conv - self.Ptype.T_h) *
#             self.Ptype.area)) / self.area
#             )
#         self.q_c_conv_flux = (
#             self.U_cold * (((self.Ntype.T_c - self.T_c_conv) *
#             self.Ntype.area) + ((self.Ptype.T_c - self.T_c_conv) *
#             self.Ptype.area)) / self.area
#             )
...
#         # individual leg heat q errors
#         self.Ntype.q_h_conv = (
#             self.U_hot * (self.T_h_conv - self.Ntype.T_h) * self.Ntype.area
#             )
#         self.Ptype.q_h_conv = (
#             self.U_hot * (self.T_h_conv - self.Ptype.T_h) * self.Ptype.area
#             )
#         self.Ntype.q_c_conv = (
#             self.U_hot * (self.Ntype.T_c - self.T_c_conv) * self.Ntype.area
#             )
#         self.Ptype.q_c_conv = (
#             self.U_cold * (self.Ptype.T_c - self.T_c_conv) * self.Ptype.area
#             )

#         Nq_c_error = self.Ntype.q_c * self.Ntype.area - self.Ntype.q_c_conv
#         Pq_c_error = self.Ptype.q_c * self.Ptype.area - self.Ptype.q_c_conv
#         Nq_h_error = self.Ntype.q_h_conv - self.Ntype.q_h * self.Ntype.area
#         Pq_h_error = self.Ptype.q_h_conv - self.Ptype.q_h * self.Ptype.area
...
#         q_h_conv_calculated = (
#             (self.Ntype.q_h * self.Ntype.area / self.area) +
#             (self.Ntype.q_h * self.Ntype.area / self.area)
#             )
#         # cold side q_c
#         self.q_c_conv_flux = (
...
#         q_c_conv_calculated = (
#             (self.Ntype.q_c * self.Ntype.area / self.area) +
#             (self.Ntype.q_c * self.Ntype.area / self.area)
#             )
#         # hot side and cold side q error
#         q_h_conv_error = self.q_h_conv_flux - q_h_conv_calculated
#         q_c_conv_error = self.q_c_conv_flux - q_c_conv_calculated

#         print "\nself.q_h_conv_flux is ", self.q_h_conv_flux
#         print "self.q_h_conv_calculated is ", q_h_conv_calculated
#         print "self.q_c_conv_flux is ", self.q_c_conv_flux
#         print "self.q_c_conv_calculated is \n", q_h_conv_calculated
...
#         self.error = (
#             np.array([Nq_c_error, Pq_c_error, Nq_h_error, Pq_h_error,
#             I_error, q_h_conv_error, q_c_conv_error]).flatten()
#             )
...
#             self.q_h_conv_flux, self.q_c_conv_flux])
#             )
...
# # This code is useless, look at the one below
# # started modifying this on Feb 28, 2013 after leaving a working copy
# # at the bottom
# import numpy as np
# from scipy.optimize import fsolve
# import time
...
#         """ """
#         print "Ntype q_h input is ", self.Ntype.q_h
#         print "Ptype q_h input is ", self.Ptype.q_h
#         self.Ntype.solve_leg_once(self.Ntype.q_h)
#         self.Ptype.solve_leg_once(self.Ptype.q_h)
#         # self.T_c = self.Ntype.T_c

#         self.Ntype.q_h_total = self.Ntype.q_h * self.Ntype.area
#         self.Ptype.q_h_total = self.Ptype.q_h * self.Ptype.area
#         self.Ntype.q_c_total = self.Ntype.q_c * self.Ntype.area
#         self.Ptype.q_c_total = self.Ptype.q_c * self.Ptype.area

#         self.q_h_conv = (
#             (self.Ntype.q_h_total + self.Ptype.q_h_total) / self.area
#             )
#         self.q_c_conv = (
#             (self.Ntype.q_c_total + self.Ptype.q_c_total) / self.area
#             )
...
#         #==================================================
#         # MOVE ALL THESE HEAT FLUXES UNDER solve_pair_once
#         #==================================================
#         Ntype_q_h_actual = (
#             self.U_hot * (self.T_h_conv - self.Ntype.T_h) * self.Ntype.area
#             )
...
#             )
#         Ntype_q_c_actual = (
#             self.U_cold * (self.Ntype.T_c - self.T_c_conv) * self.Ntype.area
#             )
#         Ptype_q_c_actual = (
#             self.U_cold * (self.Ptype.T_c - self.T_c_conv) * self.Ptype.area
#             )
        
#         # convection total heat(not flux) based on T_h and T_c of the legs
#         q_h_conv_actual = (
#             (Ntype_q_h_actual + Ptype_q_h_actual) / self.area
#             )
#         q_c_conv_actual = (
#             (Ntype_q_c_actual + Ptype_q_c_actual) / self.area
#             )

#         q_h_error = self.q_h_conv - q_h_conv_actual
#         q_c_error = self.q_c_conv - q_c_conv_actual
...
#         self.I_error = self.I_correct - self.I
#         self.error = (
#             np.array([q_h_error, q_c_error, 0.0, 0.0, self.I_error]).flatten()
#             )
#         print "Error is ", self.error
#         return self.error
...
#         knob_arr0 = (
#             np.array([self.Ntype.q_h, self.Ptype.q_h,
#             self.T_h_conv, self.T_h_conv, self.I])
#             )
